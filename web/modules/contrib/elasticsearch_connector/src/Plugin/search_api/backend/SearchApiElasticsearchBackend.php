...
class SearchApiElasticsearchBackend extends BackendPluginBase implements PluginFormInterface, SearchApiElasticsearchBackendInterface {
  ...
  public function fieldsUpdated(IndexInterface $index) {
    ...
    try {
      ...
      $response = $this->client->indices()->putMapping(
        $this->indexFactory->mapping($index, $this->getSpecialFields($index))
      );
  ...
  /**
   * Retrieves the settings for an ElasticSearch index.
   *
   * For example: analyzer, filters for stopwords and so on.
   */
  ...
  public function setSettings($index, $settings) {
    $params = $this->indexFactory->index($index);
    $params_index = $this->indexFactory->index($index);
    $this->client->indices()->close($params_index);
    $settings = $settings[$params['index']]['settings'];
    $params = $params_index;
    // Remove read-only settings, if present.
    unset($settings['index']['creation_date']);
    unset($settings['index']['provided_name']);
    unset($settings['index']['uuid']);
    unset($settings['index']['version']);
    unset($settings['index']['number_of_shards']);
    $params['body'] = $settings;


    try {
      if ($this->client->indices()->exists($params_index)) {
      ...
      $this->client->indices()->open($params_index);
    }
  }
    ...
    foreach ($facets as $key => $facet) {
      ...
      elseif ($facet['operator'] == 'or') {
        if (!empty($facet['type']) && $facet['type'] == 'stats') {
          ...
          // those of the non-global bucket.
          $position = $this->inArrayRecursive($facet['field'], $conditions);
          if ($position !== FALSE) {
            // Generate query without facet filter and get the bucket result
            // for this facet.
            $terms = $this->getFacetResult($query, $position, $facet['field']);
          }
          else {
            $buckets = $response['aggregations'][$key]['buckets'];
            array_walk($buckets, function ($value) use (&$terms) {
   ...
   * Find $needle postion.
   *
   * This function return the position of needle in $haystack, when $haystack is
   ...
   *
   * @return int|bool
   *   Return the condition number of $needle if is find or false.
   */
  ...
  protected function getFacetResult(QueryInterface $query, int $position, string $facet_name) {
    $facet_query = clone $query;
    /** @var \Drupal\search_api\Query\QueryInterface $facet_query */
    //$facet_query->removeConditionGroup($position);
    // Query conditions.
    ...
    try {
      ...
      $facets = $facet_query->getOption('search_api_facets');
      $facet = $facets[$facet_name];
      if (!empty($facet['type']) && $facet['type'] == 'stats') {
        $terms = $response['aggregations'][$facet_name];
      }
      else {
        $buckets = $response['aggregations'][$facet_name]['buckets'];
        array_walk($buckets, function ($value) use (&$terms) {
          $terms[] = [
            'count' => $value['doc_count'],
            'filter' => '"' . $value['key'] . '"',
